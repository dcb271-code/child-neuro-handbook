...
export default function RiteResults({
  exam, questions, answers, result, pgy, onRetake, onBack,
}: Props) {
  return (
              ...
              <div className="flex gap-2 items-start">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-slate-400 dark:text-slate-500 mb-1 tabular-nums">
                    {i + 1}. Item {q.num}{q.part !== 'a' ? q.part : ''}
                  </p>
                  ...
                  {q.images?.map((src) => (
                    ...
                    <img key={src} src={src} alt={q.imageAlt ?? 'Exam figure'} className="w-full max-w-md h-auto rounded-lg border border-slate-200 dark:border-slate-700 mb-2 bg-white" />
                  ))}
