...
export default async function FamilyPointsPage() {
  ...
  // Only show month columns through the latest month that has any points.
  ...
  return (
    <div>
            ...
            <li key={t.teamId} className={`fp-c-${t.colorIndex}`}>
              ...
              <div className="h-3 rounded-full bg-slate-100 dark:bg-slate-700/60 overflow-hidden ml-5">
                <div
                  ...
                  style={{ width: `${(t.total / board.maxTeamTotal) * 100}%`, backgroundColor: 'var(--fp)' }}
                />
              </div>
            </li>
          ))}
      ...
      <section className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/80 p-4 sm:p-5 mb-4">
        <h2 className="text-base font-semibold text-slate-900 dark:text-white mb-2">Members</h2>
        {board.teams.map((t) => (
          <details key={t.teamId} className={`fp-c-${t.colorIndex} group border-b border-slate-100 dark:border-slate-700/60 last:border-0`}>
            <summary className="flex items-center gap-2.5 cursor-pointer select-none py-2.5 text-sm text-slate-700 dark:text-slate-200">
              <TeamIcon teamId={t.teamId} size={22} className="shrink-0" />
              <span className="font-medium flex-1">{t.name}</span>
              <span className="text-xs text-slate-400 dark:text-slate-500 tabular-nums">{t.total} pts</span>
            </summary>
            <ul className="pb-3 pl-5 space-y-2">
              {t.members.map((m) => (
                <li key={m.name} className="text-sm">
                  <div className="flex items-baseline justify-between gap-3">
                    <span className="text-slate-700 dark:text-slate-200">
                      {m.name} <span className="text-xs text-slate-400 dark:text-slate-500">PGY{m.pgy}</span>
                    </span>
